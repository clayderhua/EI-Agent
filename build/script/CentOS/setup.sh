...
CMA_SCRIPT="/lib/systemd/system/saagent.conf"
...
config_susiaccess ()
{
	...
	if [ "$susiaccess_config_ans" == "y" ] ||[ "$susiaccess_config_ans" == "Y"  ] ||[ "$susiaccess_config_ans" == ""  ]; then
				...
				while true
				do
					read -p "Zero-touch onboard [y/n](default: $auto_conn): " autoconn
					if [ "$autoconn" == "y" ] || [ "$autoconn" == "Y" ]; then
						autoconnect="Auto"
						break
						...
						if [ "$auto_conn" == "y" ] || [ "$auto_conn" == "Y" ]; then
							autoconnect="Auto"
						else
							autoconnect="Standalone"
						fi		
				...
				if [ "$autoconnect" == "Standalone" ]; then
					...
					while true
					do
						read -p "Assign device to User Account [y/n](default: $autoassign): " auto_assign
						if [ "$auto_assign" == "y" ] || [ "$auto_assign" == "Y" ]; then
							...
							username="anonymous"
							break
						elif [ "$auto_assign" == ""  ]; then
							if [ "$autoassign" == "y" ] || [ "$autoassign" == "Y" ]; then
								username=$iotkey
							else
								username="anonymous"
							fi		
...
#	if [ $reconn == "disable" ]; then
	...
	do
		...
		if [ "$susiaccess_start_ans" == "y" ] ||[ "$susiaccess_start_ans" == "Y"  ] ||[ "$susiaccess_start_ans" == ""  ]; then
			...
			break
		elif [ "$susiaccess_start_ans" == "n" ] || [ "$susiaccess_start_ans" == "N"  ]; then
			break
		else
			echo "Input error string"
		fi
...
load_config
check_susiaccess
echo "WISE-Agent Linux setup successfully!"
