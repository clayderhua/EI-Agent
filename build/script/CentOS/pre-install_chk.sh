...
#	${RUN_PKG_NEED} is specify the depend packages to execute WISE-Agent.
#	${DEVLE_PKG_NEED} is specify the depend packages to develop WISE-Agent.
RUN_PKG_NEED="libxml2 openssl libcurl libXtst libX11 \
redhat-lsb-core xterm sqlite lua curl mosquitto"
DEVLE_PKG_NEED="gcc gcc-c++ make \
libxml2-devel openssl-devel libcurl-devel libXtst-devel libX11-devel \
mosquitto-devel autoconf libtool lua-devel"
CHK_CMD=rpm
CHK_ARG=-q
INSTALL_CMD=yum
...
function mosquitto_source_import()
{
	...
	cut_off_line
	${INSTALL_CMD} ${INSTALL_ARG} epel-release
}
