...
pre_setup_actions ()
{
	...
	$SAWATCHDOG_DAEMON stop 2>&1
	$SAAGENT_DAEMON stop  2>&1
	$SAWEBSOCKIFY_DAEMON stop 2>&1
}
post_setup_actions ()
{
	echo "sending request to start AgentService" 
	$SAWEBSOCKIFY_DAEMON start 2>&1
	$SAAGENT_DAEMON start  2>&1
	$SAWATCHDOG_DAEMON start 2>&1
}
check_linux_version ()
{
	...
	get_linux_arch
	SAWATCHDOG_DAEMON=/etc/init.d/sawatchdog
	SAWEBSOCKIFY_DAEMON=/etc/init.d/sawebsockify
	SAAGENT_DAEMON=/etc/init.d/saagent
...
#	if [ $reconn == "disable" ]; then
		echo "****************************************************************************************************"
		read -p "Do you want to start WISE-Agent now? [y/n](default: y)" RMM_start_ans
		if [ "$RMM_start_ans" == "y" ] ||[ "$RMM_start_ans" == "Y"  ] ||[ "$RMM_start_ans" == ""  ]; then
			$SAAGENT_DAEMON status >/dev/null 2>&1
			if [ $? -ne 0 ]; then
				echo -e "\nWISE-Agent Service Starting..." 
				$SAAGENT_DAEMON start
			else
				echo -e "\nWISE-Agent Service Restarting..." 
				$SAAGENT_DAEMON restart
			fi
		fi
#	fi
}
