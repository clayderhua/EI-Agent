...
# Function descript: Auto check and install the rpm's packages that you specify on openSUSE
#         arguments: -devel : Install development packages 
#           example: ./pre-install_chk.sh 		 --> install the packages that ${RUN_PKG_NEED} specify 
#                    ./pre-install_chk.sh -devel --> also install the corresponding development packages
...
RUN_PKG_NEED="libxml2-2 openssl libcurl4 libXtst6 libX11-6 libmosquitto1 \
sqlite3"
DEVLE_PKG_NEED="gcc gcc-c++ make \
 libxml2-devel libopenssl-devel libcurl-devel libXtst-devel libX11-devel \
 libmosquitto-devel"

CHK_CMD=rpm
CHK_ARG=-q
INSTALL_CMD=zypper
INSTALL_ARG="install -l" 
MOSQUITTO_SOURCE_URL="http://download.opensuse.org/repositories/home:/oojah:/mqtt/openSUSE_13.1/home:oojah:mqtt.repo"
...
function mosquitto_source_import()
{
	...
	zypper addrepo ${MOSQUITTO_SOURCE_URL}
	zypper refresh
