...
{
	fprintf(stderr, "[XMLBase] DllInitializer\n");
	xmlInitParser();
}
...
{
    fprintf(stderr, "[XMLBase] DllFinalizer\n");
	xmlCleanupParser();
}
#endif
...
xml_doc_info * SACONFIG_API xml_Loadfile(char const * configFile, char const * const xml_root, char const * const xml_base)
{
	...
	docinfo->doc = doc;
	strncpy(docinfo->xml_root, xml_root, sizeof(docinfo->xml_root));
	strncpy(docinfo->xml_base, xml_base, sizeof(docinfo->xml_base));
	return docinfo;
}
