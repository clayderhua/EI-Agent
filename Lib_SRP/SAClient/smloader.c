...
void smloader_function_load(SAManager_Interface * SAManager)
{
	if(SAManager != NULL && SAManager->Handler != NULL)
	{
		...
		SAManager->SAManager_SetUnsubscribeCB_API = (SAMANAGER_SETUNSUBSCRIBECB)dlsym(SAManager->Handler, "SAManager_SetUnsubscribeCB");
	}
}
...
void smloader_uninit()
{
	if(g_SAManager)
	{
		...
		if(g_SAManager->SAManager_SetUnsubscribeCB_API)
			g_SAManager->SAManager_SetUnsubscribeCB_API(NULL);
...
void smloader_callback_set(PUBLISHCBFN fn_publish, SUBSCRIBECBFN fn_subscribe, CONNECTSERVERCBFN fn_connserver, DISCONNECTCBFN fn_disconnect, UNSUBSCRIBECBFN fn_unsubscribe)
{
	if(!g_SAManager)
		return;
	if(g_SAManager->SAManager_SetPublishCB_API)
	...
	if(g_SAManager->SAManager_SetUnsubscribeCB_API)
		g_SAManager->SAManager_SetUnsubscribeCB_API(fn_unsubscribe);
}
