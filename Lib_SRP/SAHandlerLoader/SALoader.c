...
void HANDLERLOADER_API Loader_SetFuncCB(Callback_Functions_t* funcs)
{
	...
	if(m_plugininfo)
	{
		if(m_plugininfo->pHandlerInfo)
		{
			...
			pHandler->sendeventcbf = funcs->sendeventcbf;
			...
			pHandler->unsubscribecustcbf = funcs->unsubscribecustcbf;
		}
	}
}
...
typedef struct {
	Handler_Loader_Interface *pInterfaceCur;
	char name[256];
	pthread_t arrThreadHandler;
	bool bFinish;
	int count;
}release_handle_t;
...
{
	Handler_Loader_Interface *pInterfaceCur = NULL;
	release_handle_t *arrThreadHandler = (release_handle_t *)args;
	if(arrThreadHandler)
	{
		pInterfaceCur = arrThreadHandler->pInterfaceCur;
		if(pInterfaceCur)
		{
			if(pInterfaceCur->type!=virtual_handler)
				Handler_StopHandler( pInterfaceCur );
			...
			free(pInterfaceCur);
			arrThreadHandler->pInterfaceCur = pInterfaceCur = NULL;		
		}
		arrThreadHandler->bFinish = true;
	}
	pthread_exit(0);
	return 0;
}
...
void HANDLERLOADER_API Loader_ConcurrentReleaseAllHandler(Handler_List_t *pLoaderList)
{
	Handler_Loader_Interface *pInterfaceTmp = NULL;
	int total = 0;
	release_handle_t *arrThreadHandler = NULL;
	int i=0;
	if(!pLoaderList) return;
	//SALoaderLog(Debug, "Loader_ConcurrentReleaseAllHandler!");
	total = pLoaderList->total;
	arrThreadHandler = calloc(total, sizeof(release_handle_t));
	pInterfaceTmp = Loader_GetLastHandler(pLoaderList);
	while(pInterfaceTmp)
	{
		Handler_Loader_Interface *pInterfaceCur = pInterfaceTmp;
		pInterfaceTmp = pInterfaceTmp->prev;
		//SALoaderLog(Debug, " %s> release thread!", pInterfaceCur->Name);
		arrThreadHandler[i].pInterfaceCur = pInterfaceCur;
		strcpy(arrThreadHandler[i].name, pInterfaceCur->Name);
		if (pthread_create(&arrThreadHandler[i].arrThreadHandler, NULL, HandlerReleaseThread, &arrThreadHandler[i]) != 0)
		{
	...
	/*Wait ReleaseHandlerThread*/
	{
		while(true)
		{
			...
			for (i = 0; i < total; i++) 
			{
				if(arrThreadHandler[i].arrThreadHandler)
				{
					if (arrThreadHandler[i].bFinish)
					{
						SALoaderLog(Debug, " %s> Released!", arrThreadHandler[i].name);
						//pthread_cancel(arrThreadHandler[i].arrThreadHandler);
						pthread_join(arrThreadHandler[i].arrThreadHandler, NULL);
						arrThreadHandler[i].arrThreadHandler = NULL;
					}
					else
					{
						//if(arrThreadHandler[i].count < 100)
							bFinish = false;
						arrThreadHandler[i].count++;
						//SALoaderLog(Debug, " %s> Not released thread! %d", arrThreadHandler[i].name, arrThreadHandler[i].count);
					}
				}
			}
			if (bFinish)
		...
		/*for (i = 0; i < total; i++) 
		{
			if(arrThreadHandler[i].arrThreadHandler)
			{
				SALoaderLog(Debug, " %s> Wait release thread!", arrThreadHandler[i]. name);
				pthread_detach(arrThreadHandler[i].arrThreadHandler);
				arrThreadHandler[i].arrThreadHandler = NULL;
			}
		}*/
	}
