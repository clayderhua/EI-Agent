...
{
	BOOL bRet = FALSE;
	if (lpSubKey == NULL || lpValueName == NULL || lpValue == NULL)
		return bRet;
	return bRet;
}
...
BOOL app_os_Str2Tm(const char *buf, const char *format, struct tm *tm)
{
	...
	if (NULL == buf || NULL == format || NULL == tm)
		return bRet;
...
BOOL app_os_FileIsUnicode(char *fileName)
{
	...
	fclose(fp);
	if (headBuf[0] == 0xFF && headBuf[1] == 0xFE)
		bRet = TRUE;
	return bRet;
}
...
int app_os_thread_equal(CAGENT_THREAD_ID threadIdA, CAGENT_THREAD_ID threadIdB)
{
	int iRet = 0;
	//Add code
	if (!pthread_equal(threadIdA, threadIdB))
		iRet = -1;
	return iRet;
}
...
int app_os_set_executable(char *path)
{
	...
	if (access(path, 0) != 0)
		return -1;
#ifdef ANDROID
	return chmod(path, S_IRUSR | S_IWUSR | S_IXUSR);
#else
...
BOOL app_os_GetRandomStr(char *randomStrBuf, int bufSize)
{
	...
	if (NULL == randomStrBuf || bufSize <= 0)
		return FALSE;
...
{
	if (x11vncPath == NULL || runParams == NULL)
		return -1;
	{
		pid_t fpid = 0;
#ifdef ANDROID
...
{
	BOOL bRet = FALSE;
	if (fHandle == NULL || buf == NULL || nSizeRead == NULL)
		return bRet;
	{
...
{
	BOOL bRet = FALSE;
	if (fHandle == NULL || buf == NULL || nSizeWrite == NULL)
		return bRet;
	{
...
BOOL app_os_file_copy(const char *srcFilePath, const char *destFilePath)
{
	...
	if (NULL == srcFilePath || NULL == destFilePath)
		goto done;
...
BOOL app_os_GetSysLogonUserName2(char *userNameBuf, unsigned int bufLen)
{
	int i = 0;
...
BOOL app_os_GetSysLogonUserName(char *userNameBuf, unsigned int bufLen)
{
	...
	bool bRet = app_os_GetSysLogonUserName2(userNameBuf, bufLen);
	if (bRet)
		return bRet;
