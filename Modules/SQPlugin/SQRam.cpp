...
bool RefreshRAMInfoList(sqram_info_list ramInfoList, int count)
{
		...
		return bRet;
	{
		...
		SQLog(Debug, "wmi_GetMemorySpeed()");
		...
		while (index < count)
		{
			...
			SQLog(Debug, "wmi_FindRAMSpeed()");
			ramInfo.Speed = wmi_FindRAMSpeed(tmplist, ramInfo.Index);
			SQLog(Debug, "RAM %d Speed: %d", ramInfo.Index, ramInfo.Speed);
...
void sqr_UpdateSQRAM(sqram_info_t* ramInfo, MSG_CLASSIFY_T* myCapability)
{
	...
	if (ramInfo && ramInfo->sqramInfoList && sqramGroup)
	{
		...
		while (item != NULL)
		{
			...
			sprintf(cTemp, "RAM%d", item->Index);
			...
			IoT_SetDoubleValueWithMaxMin(attr, item->Temperature, IoT_READONLY, 100, -40, SENSOR_UNIT_CELSIUS);
...
bool sqr_HasSQRam(sqram_info_t* pRamInfo)
{
	bool bFound = false;
	sqram_node_t* head = NULL;
	if (pRamInfo->sqramInfoList == NULL)
		return bFound;
	head = pRamInfo->sqramInfoList;
	while (head)
	{
		if (strstr(head->SerialNumber, "SQR-") > 0) //Filter SQRAM
		{
			bFound = true;
			break;
		}
