...
int launchChild(int service, struct Session *session)
{
  ...
  ssize_t len = sizeof(struct LaunchRequest) + 1;
  ...
  session->pid = pid;
  //printf("launchChild end, pty:%d, pid:%d\n", session->pty, pid);
  return pid;
}
...
static struct HashMap *childProcesses;
...
static int forkPty(int *pty, int useLogin, struct Utmp **utmp, const char *peerName)
{
  int slave;
#ifdef HAVE_OPENPTY
  char *ptyPath = NULL;
      ...
      (slave = NOINTR(open(ptyPath, O_RDWR | O_NOCTTY))) < 0)
  {
    ...
    const char *ptr1 = "pqrstuvwxyzabcde";
    if (*pty >= 0)
    {
      NOINTR(close(*pty));
    }
    ...
    for (; *ptr1; ptr1++)
    {
      const char *ptr2 = "0123456789abcdef";
      fname[8] = *ptr1;
      for (; *ptr2; ptr2++)
      {
        fname[9] = *ptr2;
  ...
  strncat(passwd->pw_shell = strrchr(
                                 strncat(passwd->pw_dir = strrchr(
                                                              ...
                                                              strncat(passwd->pw_gecos = strrchr(
                                                                                         ...
                                                                                         1,
#ifndef ANDROID
                                                                      pw->pw_gecos, strlen(pw->pw_gecos)),
                                                              '\000') +
                                                          1,
#endif
                                   ...
                                   char ***environment)
{
  ...
  if (service->useDefaultShell)
  {
    ...
    service->cmdline = strdup(*pw->pw_shell ? pw->pw_shell : "/bin/sh");
  }
                         ...
                         const char *url)
{
  ...
  {
#ifdef ANDROID
    //execle("/system/bin/busybox", "busybox", "login", (void *)0, environment);
    //printf("execle busybox login \n");
    execle("/system/bin/sh", "sh", (void *)0, environment);
    printf("execle sh \n");

#else

    execle("/bin/login", "login", "-p", (void *)0, environment);
    execle("/usr/bin/login", "login", "-p", (void *)0, environment);
    printf("execle login E\n");
#endif
  }
  else
  {
