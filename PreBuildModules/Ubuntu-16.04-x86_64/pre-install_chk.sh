...
sqlite3 xterm ethtool lua5.1 net-tools"

if [[ `lsb_release -rs` == "18.04" ]]; then
	RUN_PKG_NEED="${RUN_PKG_NEED} openssl1.0 libcurl3"
fi

# ${DEVLE_PKG_NEED} is specify the depend packages to develop RMM cagent.
DEVLE_PKG_NEED="gcc g++ make \
libxml2-dev libx11-dev libxtst-dev libxext-dev \
libmosquitto-dev autoconf autotools-dev build-essential libtool"

if [[ `lsb_release -rs` == "16.04" ]]; then
	DEVLE_PKG_NEED="${DEVLE_PKG_NEED} libcurl4-openssl-dev libssl-dev"
elif [[ `lsb_release -rs` == "18.04" ]]; then
	DEVLE_PKG_NEED="${DEVLE_PKG_NEED} libcurl-openssl1.0-dev libssl1.0-dev"
else
	DEVLE_PKG_NEED="${DEVLE_PKG_NEED} libcurl4-openssl-dev"
fi
...
if [ "x${PKG_NEED_INSTALL// /}" != "x" ]; then
	...
	install_pkg
	if [ $# -ne 0 ] && [ "$1" = "-devel" ]; then
		sudo ln -s /usr/lib/x86_64-linux-gnu/liblua5.1.so.0.0.0 /usr/lib/x86_64-linux-gnu/liblua.so
	fi
