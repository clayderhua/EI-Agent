...
static int CheckBase64(const char * data, int dataLen)
{
	int iRet = -1;
	if(NULL == data || dataLen <= 0)return iRet;
	{
		int i = 0;
		for(i = 0; i<dataLen; i++)
		{
			char curCh = data[i];
			if(strchr(base, curCh))
			{
				if(curCh == '=')
				{
					if(((i == dataLen -2) && data[i+1] != '=') || i < dataLen -2)
					{
						break;
					}
				}
...
int Base64Decode(IN const char * data, IN int dataLen, OUT char ** decData, OUT int * decLen)
{
   ...
   if(NULL == data || dataLen <= 0 || NULL == decData || NULL == decLen) return iRet;	
	...
	if(CheckBase64(data, dataLen) != 0) return iRet = 2;
