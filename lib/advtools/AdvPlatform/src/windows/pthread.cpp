...
pthread_t ADVPLAT_CALL pthread_self(void) {
	return GetCurrentThreadId();
}

int ADVPLAT_CALL pthread_detach(pthread_t thread) {
	HANDLE h = OpenThread(THREAD_QUERY_INFORMATION, FALSE, thread);
	CloseHandle(h);
	return 0;
}

int ADVPLAT_CALL pthread_cancel(pthread_t thread)
{
	int iRet = 0;
	HANDLE h = OpenThread(THREAD_QUERY_INFORMATION, FALSE, thread);
...
void ADVPLAT_CALL pthread_exit(void* retval)
{
...
int ADVPLAT_CALL pthread_cond_init(pthread_cond_t *cond, const pthread_condattr_t *attr)
{
	int iRet = 0;
...
int ADVPLAT_CALL pthread_cond_destroy(pthread_cond_t *cond)
{
	int iRet = 0;
