...
/*0*/ADVLOG_EXPORT int ADVLOG_CALL AdvLog_Status(char *ConfName, int level);
...
/*K*/ADVLOG_EXPORT time_t ADVLOG_CALL AdvLog_TimesUp(time_t check, time_t interval);

ADVLOG_EXPORT void ADVLOG_CALL AdvLog_ElsWrite( const char* DataPath,  const char *data, int len );
#ifdef ANDROID
#define AdvLog_TimesUp(check,interval) ({time_t p; p;})
#endif
#define __ADV_PRINT_STATUS_NONE	0
#define __ADV_PRINT_STATUS_P	1
#define __ADV_PRINT_STATUS_W	2
#define __ADV_PRINT_STATUS_PW	__ADV_PRINT_STATUS_P|__ADV_PRINT_STATUS_W
...
#define __ADV_PRINT(level, color, colorstr, colorend, fmt,...) 	do {\
												...
												if (!status) break;\
												switch (status) {\
													case __ADV_PRINT_STATUS_P:   AdvLog_Print(NULL, level, color,__FILE__, __func__, S__LINE__,#level, fmt, ##__VA_ARGS__);   break;\
													case __ADV_PRINT_STATUS_W:   AdvLog_Write(NULL, level, colorstr,colorend,__FILE__, __func__, S__LINE__,#level, fmt, ##__VA_ARGS__);   break;\
													case __ADV_PRINT_STATUS_PW:   AdvLog_PrintAndWrite(NULL, level, color, colorstr, colorend, __FILE__, __func__, S__LINE__,#level, fmt, ##__VA_ARGS__);   break;\
												}\
											} while(0)

#define __ADV_PRINT_BRANCH(name, level, color, colorstr, colorend, fmt,...) 	do {\
												int status = AdvLog_Status(name, level);\
												if (!status) break;\
												switch (status) {\
													case __ADV_PRINT_STATUS_P:   AdvLog_Print(name, level, color,__FILE__, __func__, S__LINE__,#level, fmt, ##__VA_ARGS__);   break;\
													case __ADV_PRINT_STATUS_W:   AdvLog_Write(name, level, colorstr,colorend,__FILE__, __func__, S__LINE__,#level, fmt, ##__VA_ARGS__);   break;\
													case __ADV_PRINT_STATUS_PW:   AdvLog_PrintAndWrite(name, level, color, colorstr, colorend, __FILE__, __func__, S__LINE__,#level, fmt, ##__VA_ARGS__);   break;\
												}\
											} while(0)
