...
typedef struct{
	...
	THRESHOLD_ON_TRIGGER on_trigger;
	void* threadHandler;
	bool isThreadRunning;
	post_report_data_cb postReportDataCB;
}handler_kernel_t;
...
void* HandlerKernelEx_MonitorThread(void *args)
{
	...
	while(true)
	{
		...
		if(pHandlerKernel->AutoReportContex.enable)
		{
			...
			pHandlerKernel->AutoReportContex.nextTime = 0;
		}
		...
		if(pHandlerKernel->pCapability)
		{
			if(bAutoReport)
			{
				...
				if (pHandlerKernel->postReportDataCB) {
					pHandlerKernel->postReportDataCB();
				}
			}
		}
			...
			if(bLiveReport)
			{
				...
				if(pHandlerKernel->LiveReportContex.reqAll)
				{
					if(pHandlerKernel->LiveReportContex.bFlat)
						...
						buff = IoT_PrintLiveData(pHandlerKernel->pCapability);
				}
				else
				{
					if(pHandlerKernel->LiveReportContex.bFlat)
						...
						buff = IoT_PrintSelectedLiveData(pHandlerKernel->pCapability, pHandlerKernel->LiveReportContex.reqItems);
				}
				pthread_mutex_unlock(&pHandlerKernel->CapabilityMux);
		...
		if(pHandlerKernel->pCapability)
		{
			if(pHandlerKernel->pThresholdList != NULL && bChkThreshold)
			{
				...
				if(buff)
				{
					if(pHandlerKernel->HandlerInfo.sendeventcbf)
				...
				HandlerParser_QueueingAction(pHandlerKernel->pCapability, pHandlerKernel->pThresholdList, pHandlerKernel->pTriggerQueue, triggerqueue_push);
			...
			if(nNode == NULL)
			 ...
			 else if(pHandlerContex->on_sensor_cmd)
			 {
				 if(strstr(attr->readwritemode,"r")>0)
				 {
					 ...
					 strncpy(target->sensorname, nNode->valuestring, sizeof(target->sensorname));
					 target->attr = attr;
...
void* HANDLERKERNEL_API HandlerKernelEx_Initialize( HANDLER_INFO *handler )
{
	...
	handler->MSgTransform = MSG_Format_EI; //setup flag to disable transform to old format.
	...
	pHandlerKernel->AutoReportContex.reqAll = true;
...
/*
	if (pHandlerKernel->pTriggerQueue == NULL)
...
/* **************************************************************************************
*  Function Name: HandlerKernelEx_Start
*  Description: Start internal thread of this HandlerKernel
*  Input : void* pHandle
*  Output: None
*  Return:  0 : Success
*          -1 : Fail
* ***************************************************************************************/
int HANDLERKERNEL_API HandlerKernelEx_Start(void* pHandle)
{
	handler_kernel_t* pHandlerKernel = NULL;
	if (pHandle == NULL)
		return handler_fail;
	...
	if(!pHandlerKernel->isThreadRunning || pHandlerKernel->threadHandler == NULL)
	{
		pHandlerKernel->isThreadRunning = true;
		if (pthread_create(&pHandlerKernel->threadHandler, NULL, HandlerKernelEx_MonitorThread, pHandlerKernel) != 0)
		{
	...
	pHandlerKernel->Status = handler_status_start;
	return handler_success;
}

/* **************************************************************************************
*  Function Name: HandlerKernelEx_Stop
*  Description: Stop internal thread of this HandlerKernel
*  Input : void* pHandle
*  Output: None
*  Return:  0 : Success
*          -1 : Fail
* ***************************************************************************************/
int HANDLERKERNEL_API HandlerKernelEx_Stop(void* pHandle)
{
	handler_kernel_t* pHandlerKernel = NULL;
	if (pHandle == NULL)
		return handler_fail;
	...
	pHandlerKernel->Status = handler_status_stop;
	return handler_success;
}
...
int HANDLERKERNEL_API HandlerKernelEx_SetThreshold(void* pHandle, int replyID, char *pInQuery)
{
	...
	tmpThrItemList->thrItemInfo.isTriggered = false;
	//tmpThrItemList->thrItemInfo.pTriggerQueue = pHandlerKernel->pTriggerQueue;
	if(!HandlerParser_ParseThrInfo(pInQuery, tmpThrItemList))
	{
	...
	if(pHandlerKernel->pThresholdList == NULL)
	{
		...
		pHandlerKernel->pThresholdList->thrItemInfo.isTriggered = false;
...
int HANDLERKERNEL_API HandlerKernelEx_StartThresholdCheck(void* pHandle)
{
	...
	{
		pHandlerKernel->pTriggerQueue = triggerqueue_init(10);
		triggerqueue_setcb(pHandlerKernel->pTriggerQueue, pHandlerKernel->on_trigger);
	}
	else
		KernelHExLog(pHandlerKernel->pHandlerkernellog, Error, " %s> Trigger Queue is Exist!", pHandlerKernel->HandlerInfo.Name);
													...
													void *pRev))
{
	...
	pHandlerKernel->on_trigger = on_triggered;
...
int HANDLERKERNEL_API HandlerKernelEx_GetSensorData(void* pHandle, int replyID, char* sessionID, char *pInQuery, bool (*on_get_sensor)(get_data_t* getlist, void *pRev))
{
	...
	getcmd = (sensor_cmd_t*)calloc(1, sizeof(sensor_cmd_t));
	strncpy(getcmd->sessionID, sessionID, sizeof(getcmd->sessionID));
...
int HANDLERKERNEL_API HandlerKernelEx_SetSensorData(void* pHandle, int replyID, char* sessionID, char *pInQuery, bool (*on_set_sensor)(set_data_t* setlist, void *pRev))
{
	...
	setcmd = (sensor_cmd_t*)calloc(1, sizeof(sensor_cmd_t));
	strncpy(setcmd->sessionID, sessionID, sizeof(setcmd->sessionID));
...
// set the callback function after report data, to clear local cache in plugin
int HANDLERKERNEL_API HandlerKernelEx_SetPostReportDataCB(void* pHandle, post_report_data_cb cb)
{
	handler_kernel_t* pHandlerKernel = NULL;
	if (!pHandle) {
		return -1;
	...
	pHandlerKernel = (handler_kernel_t*)pHandle;
	pHandlerKernel->postReportDataCB = cb;
	return 0;
}
