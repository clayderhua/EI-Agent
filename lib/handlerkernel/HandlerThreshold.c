...
thr_item_list HANDLERTHRESHOLD_API HandlerThreshold_CreateThrList()
{
	...
	if(head)
	{
		...
		head->thrItemInfo.maxThr = 0;
		head->thrItemInfo.minThr = 0;
		...
		head->thrItemInfo.checkRetValue = 0;
		...
		head->thrItemInfo.repThrTime = DEF_INVALID_TIME;
		head->thrItemInfo.isNormal = true;
		head->thrItemInfo.isInvalid = true;
		head->thrItemInfo.isTriggered = false;
		//head->thrItemInfo.on_triggered = NULL;
		//head->thrItemInfo.pTriggerQueue = NULL;
	}
	return head;
}
...
bool HANDLERTHRESHOLD_API HandlerThreshold_CloneActionList(thr_action_t* pActionList, thr_action_t** pNewActionList)
{
	thr_action_t* lastAction = NULL;
	thr_action_t* pAction = pActionList;

	if(pActionList == NULL || pNewActionList == NULL)
		return false;

	if(*pNewActionList != NULL)
	{
		thr_action_t* pNext = NULL;
		thr_action_t* pAct = *pNewActionList;
		*pNewActionList = NULL;
	...
	while(pAction)
	{
		thr_action_t* newAction = (thr_action_t *)malloc(sizeof(thr_action_t));
		memcpy(newAction, pAction, sizeof(thr_action_t));

		//strcpy(newAction->pathname, pAction->pathname);
		
		//newAction->type = pAction->type;
		switch(newAction->type)
		{
			case action_type_numeric:
				newAction->v = pAction->v;
				break;
			case action_type_boolean:
				newAction->bv = pAction->bv;
				break;
			case action_type_string:
				if(pAction->sv != NULL)
				{
					int size = strlen(pAction->sv);
					if(size > 0)
					{
						newAction->sv = calloc(1, size+1);
						strcpy(newAction->sv, pAction->sv);
					}
				}
				break;
		}
		if(*pNewActionList == NULL)
			*pNewActionList = lastAction = newAction;
		else
		{
...
bool HANDLERTHRESHOLD_API HandlerThreshold_InsertThrList(thr_item_list thrList, thr_item_info_t * pThrItem)
{
	...
	if(findNode == NULL)
	{
		...
		newNode->thrItemInfo.checkRetValue = 0;
		...
		newNode->thrItemInfo.isTriggered = pThrItem->isTriggered;
		//newNode->thrItemInfo.on_triggered = pThrItem->on_triggered;
		//newNode->thrItemInfo.pTriggerQueue = pThrItem->pTriggerQueue;
		HandlerThreshold_CloneActionList(pThrItem->pActionList, &newNode->thrItemInfo.pActionList);
	...
	if(NULL == newThrList || NULL == curThrList || checkRetMsg == NULL || bufLen == 0) return bRet;
	{
		...
		while(newThrItemNode)  //merge old&new thr list
		{
			...
			if(findThrItemNode) //exist then update thr argc
			{
				...
				findThrItemNode->thrItemInfo.isTriggered = newThrItemNode->thrItemInfo.isTriggered; 
				//findThrItemNode->thrItemInfo.on_triggered = newThrItemNode->thrItemInfo.on_triggered; 
				//findThrItemNode->thrItemInfo.pTriggerQueue = newThrItemNode->thrItemInfo.pTriggerQueue;
				HandlerThreshold_CloneActionList(newThrItemNode->thrItemInfo.pActionList, &findThrItemNode->thrItemInfo.pActionList);
			}
...
bool HandlerThreshold_CheckThrItem(thr_item_info_t * pThrItemInfo, MSG_ATTRIBUTE_T* attr, char * checkRetMsg)
{
	...
	{
		if(HandlerThreshold_CheckSrcVal(pThrItemInfo, attr->v))
		{  
			if(pThrItemInfo->checkSrcValList.nodeCnt <= 0)
				return bRet;
	...
	{
		if(HandlerThreshold_CheckSrcVal(pThrItemInfo, attr->bv?1:0))
		{  
			if(pThrItemInfo->checkSrcValList.nodeCnt <= 0)
				return bRet;
			if(pThrItemInfo->rangeType & range_max)
			{
				...
				{
					//sprintf(tmpRetMsg, "%d(%s:%.0f)>maxThreshold(%.0f)", pThrItemInfo->id, checkTypeStr, pThrItemInfo->checkRetValue.vf, pThrItemInfo->maxThr);
					sprintf(tmpRetMsg, "%s(%s:%s)>%s(%s)", pThrItemInfo->pathname, checkTypeStr, pThrItemInfo->checkRetValue>0?"true":"false", DEF_MAX_THR_EVENT_STR, pThrItemInfo->maxThr>0?"true":"false");
					if(strlen(descEventStr)) sprintf(tmpRetMsg, "%s:%s", tmpRetMsg, descEventStr);
					triggerMax = true;
				}
			}
			if(pThrItemInfo->rangeType & range_min)
			{
				if(pThrItemInfo->checkRetValue < pThrItemInfo->minThr)
				{
					//if(strlen(tmpRetMsg)) sprintf(tmpRetMsg, "%s and %d(%s:%.0f)<minThreshold(%.0f)", tmpRetMsg, pThrItemInfo->id, checkTypeStr, pThrItemInfo->checkRetValue.vf, pThrItemInfo->minThr);
					//else sprintf(tmpRetMsg, "%d(%s:%.0f)<minThreshold(%.0f)", pThrItemInfo->id, checkTypeStr, pThrItemInfo->checkRetValue.vf, pThrItemInfo->minThr);
					if(strlen(tmpRetMsg)) sprintf(tmpRetMsg, "%s %s %s(%s:%s)<%s(%s)", tmpRetMsg, DEF_AND_EVENT_STR, pThrItemInfo->pathname, checkTypeStr, pThrItemInfo->checkRetValue==1?"true":"false", DEF_MIN_THR_EVENT_STR, pThrItemInfo->minThr==1?"true":"false");
					else sprintf(tmpRetMsg, "%s(%s:sf)<%s(%s)", pThrItemInfo->pathname, checkTypeStr, pThrItemInfo->checkRetValue>0?"true":"false", DEF_MIN_THR_EVENT_STR, pThrItemInfo->minThr>0?"true":"false");
					if(strlen(descEventStr)) sprintf(tmpRetMsg, "%s:%s", tmpRetMsg, descEventStr);
					triggerMin = true;
				}
			}
	...
	if(curThrItemList == NULL || pCapability == NULL || checkRetMsg == NULL || isNormal == NULL || bufLen == 0) return bRet;
	{
		...
		while(curThrItemNode)
		{
			if(curThrItemNode->thrItemInfo.isEnable)
			{
				...
				if(attr == NULL || (attr->type != attr_type_numeric && attr->type != attr_type_boolean))
				{
					/*
